...
class SpinGlassEnergy:
    ...
    def _construct_ising_hamiltonian(self, laplacian, J, offset=0):
        N = laplacian.shape[0]
        # Pauli z Matrix
        sz = [[1., 0.], [0., -1.]]
        # create graph
        ...
        for i in range(N):
            for j in range(i, N):
                if laplacian[i,j] != 0.: edges.append([i, j])
    ...
    def laplacian_to_hamiltonian(self, laplacian, J):
        hamiltonian, graph, hilbert = self._construct_ising_hamiltonian(laplacian, J)
